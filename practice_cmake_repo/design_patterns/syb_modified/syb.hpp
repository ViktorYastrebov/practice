...
struct Result {
  typedef typename STDFunc::result_type Type;
};
...
//mine
template<typename Result_, typename P1_, typename P2_>
struct Param1<std::function<Result_(P1_, P2_)> > {
  typedef P1_ Type;
};
//-mine
...
Param_& gmapT(Fun_ fun, Param_& param) {
  //return gfold(ApplyToSecond<Fun_, Param_&>(fun), Bind<Param_&>(param), param);
  return gfold(ApplyToSecond<Fun_, Param_ &>(fun), std::bind<Param_>(param), param);
}
...
template<typename STDFun_>
struct MkT {
  STDFun_ fun_;
  MkT(STDFun_ fun) : fun_(fun) {}
  
  template<typename Param_>
  Param_ & operator()(Param_& param) {
    return param;
  }
  typename Param1<std::function<STDFun_>>::Type operator()(typename Param1<std::function<STDFun_>>::Type param) {
    return fun_(param);
  }
};
...
template<typename Result_, typename P1_, typename P2_>
struct Curry <std::function<Result_(P1_, P2_)> > {
  //typedef Result_ Result;
  typedef Result_ result_type;
  typedef P2_ Param1;
  std::function<Result_(P1_, P2_)> fun_;
  P1_ value_;

  Curry(std::function<Result_(P1_, P2_)> fun, P1_ value)
    :fun_(fun),
    value_(value)
  {}

  result_type operator()(Param1 param1) const {
    return fun_(value_, param1);
  }
};
...
typedef Salary &(*functype)(float k, Salary& salary);

Curry<std::function<Salary &(float, Salary&)>> incS(float k) {
  std::function <Salary &(float, Salary&)> funcIncS = static_cast<functype>(incS);
  return Curry<std::function<Salary &(float, Salary&)> >(funcIncS, k);
